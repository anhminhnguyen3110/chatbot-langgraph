...
https://github.com/ibbybuilds/agent/issues/99
"""
...
class TestEventsMode:
    ...
    async def test_events_mode_yields_raw_events(self):
        ...
        assert len(events_yielded) > 0, (
            "Expected at least one 'events' event to be yielded"
        )

        # Check that we got the raw events
        event_types = [payload.get("event") for _, payload in events_yielded]
        assert (
            "on_chain_start" in event_types
            or "on_chain_stream" in event_types
            or "on_chain_end" in event_types
        )
    ...
    async def test_events_mode_with_on_chain_stream_events(self):
        ...
        async def mock_astream_events(*args, **kwargs):
            ...
            yield {
                ...
                "run_id": "run-123",
                "data": {
                    "chunk": ("debug", {"type": "checkpoint", "payload": {"tasks": []}})
                },
            }
        ...
        print(
            f"DEBUG: Events payloads event types: {[p.get('event') for _, p in events_yielded]}"
        )

        # Check if we got on_chain_stream events as raw events
        on_chain_stream_events = [
            p for _, p in events_yielded if p.get("event") == "on_chain_stream"
        ]
