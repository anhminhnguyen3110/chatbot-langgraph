...
This is the first service layer implementation in Agent. The pattern will be
...
class AssistantService:
    ...
    async def create_assistant(
        ...
        if graph_id not in available_graphs:
            raise HTTPException(
                400,
                f"Graph '{graph_id}' not found in agent.json. Available: {list(available_graphs.keys())}",
            )
