...
from .async_loader import AsyncModuleLoader
...
class LangGraphService:
    ...
    def __init__(self, config_path: str = "agent.json"):
        # Default path can be overridden via AGENT_CONFIG or by placing agent.json
    ...
    async def initialize(self):
        ...
        Resolution order:
        1) AGENT_CONFIG env var (absolute or relative path)
        ...
        # 3) agent.json if present
        elif Path("agent.json").exists():
            resolved_path = Path("agent.json")
        ...
        if not resolved_path.exists():
            raise ValueError(
                ...
                "AGENT_CONFIG path, ./agent.json, or ./langgraph.json"
            )
        ...
        # Register async loaders from config
        self._register_async_loaders()
    ...
    def _load_graph_registry(self):
        """Load graph definitions from agent.json"""
        for graph_id, config in self.config.get("graphs", {}).items():
            # Parse path (support both "path:export" and {"path": "..."})
            if isinstance(config, str):
                graph_path, async_loaders = config, None
            else:
    ...
    def _register_async_loaders(self):
        """Register async loaders from all graphs"""
        loaders = {}
